...
public class SingleLaneMultipleCars {
...
//            speed[i] = indexStart;
//            safety_gap[i] = indexStart+NUMBER_OF_VEHICLES+i;
//            accel[i] = indexStart+2*NUMBER_OF_VEHICLES+i;
//            if (i < NUMBER_OF_VEHICLES -1){
//                distance[i] = indexStart+3*NUMBER_OF_VEHICLES+i;
//            }
//        }
//        if (INIT_SPEED.length != NUMBER_OF_VEHICLES || INIT_ACCEL.length != NUMBER_OF_VEHICLES || INIT_DISTANCE_BETWEEN.length != NUMBER_OF_VEHICLES -1) {
//            System.out.println("WARNING: Initial vehicle values are too many or too few");
//        }
//    }
    ...
    private static final double DRUNK_DRIVER_CHANCE = 0.5;
    ...
    private static Perturbation getDrunkDriverPerturbation(int timesToApply, int frequency) {
        return new IterativePerturbation(timesToApply, new AtomicPerturbation(frequency, SingleLaneMultipleCars::applyDrunkDriverPerturbation));
    }
    ...
    private static Perturbation getBreakCheckPerturbation(int timesToApply, int frequency) {
        return new IterativePerturbation(timesToApply, new AtomicPerturbation(frequency, SingleLaneMultipleCars::applyBreakCheckPerturbation));
    }

    private static DataState applyBreakCheckPerturbation(RandomGenerator rg, DataState state){
        List<DataStateUpdate> updates = new LinkedList<>();

        // Each uncontrolled vehicle has a BREAK_CHECK_CHANCE probability of doing a break check
